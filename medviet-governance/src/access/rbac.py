...
def require_permission(resource: str, action: str):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            # FastAPI injects current_user as positional arg (Depends injects it
            # before the function's own args). Since FastAPI resolves Depends
            # parameters and passes them positionally before wrapper args, we
            # check positional args first, then kwargs.
            current_user = None
            if args:
                current_user = args[0] if isinstance(args[0], dict) else None
